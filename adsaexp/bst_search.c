...
void main()
{
    ...
    int x;
    printf("\nEnter the number to be searched:");       //input to be searched
    scanf("%d",&x);

    if(Search(root,x))              //output by search function
    {
...
struct node* Search(struct node* root, int key)     //traverse and compare the value to each node
{
